...
MANIFEST_PATH = REPO_ROOT / "SEED_PACKAGE_MANIFEST.yaml"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agent_x-seed-") as tmpdir:
        ...
        _copy_tree(REPO_ROOT / "scripts", target / "scripts")

        for d in ["tests/seed_l0", "requirements"]:
            src = REPO_ROOT / d
            if src.exists():
        ...
        inv = _run(
            [sys.executable, "-m", "pytest", "tests/seed_l0/", "-q", "--tb=short"],
            target,
        )
