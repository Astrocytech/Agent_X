...
def validate_seed_package_manifest() -> dict[str, Any]:
    path = ROOT / "L0/manifests/SEED_PACKAGE_MANIFEST.yaml"
...
def validate_capability_manifest() -> None:
    path = ROOT / "L0/manifests/CAPABILITY_MANIFEST.yaml"
    manifest = load_yaml(path)
...
def validate_manifest_closure(manifest: dict[str, Any]) -> None:
    ...
    for py in Path(ROOT / "L0/CODE").rglob("*.py"):
        if "__pycache__" in py.parts:
            continue
        rel = py.relative_to(ROOT).as_posix()
        if rel not in manifest_files:
            raise AssertionError(
                f"L0/CODE python file not in SEED_PACKAGE_MANIFEST.yaml: {rel}"
            )
...
def validate_test_boundary() -> None:
    test_dir = ROOT / "L0/tests/seed_l0"
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if root in ("extensions", "examples"):
                        raise AssertionError(
                            f"L0/tests/seed_l0/{py.name} imports extension code: {root}"
                        )
            elif isinstance(node, ast.ImportFrom):
                if node.module is None:
                    continue
...
def validate_seed_tool_registry() -> None:
    seed_tools_dir = ROOT / "L0/CODE" / "tool_gateway" / "seed_tools"
...
def validate_blocked_capabilities() -> None:
    ...
    capability = load_yaml(ROOT / "L0/manifests/CAPABILITY_MANIFEST.yaml")
