    ...
    if isinstance(data, list):
        for i, entry in enumerate(data):
            if "review_id" not in entry:
                errors.append(f"Entry {i}: missing review_id")
            if "decision" not in entry:
                errors.append(f"Entry {i}: missing decision")
    return errors

def main():
    path = sys.argv[1] if len(sys.argv) > 1 else ".agentx-init/five_document_closure/final/five_document_review_record_validation.json"
