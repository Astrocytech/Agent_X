"""Append a real command result to the adapter command transcript.

Usage:
    python3 adapter_record_command.py -- <command>

Runs <command> as a subprocess, appends its CommandResult to
.agentx-init/reports/adapter-mvp/adapter_command_transcript.json,
...
TRANSCRIPT_PATH = REPORT_DIR / "adapter_command_transcript.json"
...
def _compute_provenance_id(cmd: str, exit_code: int, stdout_hash: str,
    ...
    fields = [
        cmd, str(exit_code), stdout_hash, stderr_hash,
...
def append_command(cmd: str, exit_code: int, stdout_full: str,
    ...
    entry = {
        ...
        "stdout_log": str(stdout_log.relative_to(REPORT_DIR.parent.parent)),
        "stderr_log": str(stderr_log.relative_to(REPORT_DIR.parent.parent)),
        "cmd_index": cmd_index,
        "timestamp": timestamp,
        ...
        "source": "subprocess",
        "recorded_by": "adapter_record_command.py",
...
def main() -> int:
    if len(sys.argv) < 1:
        print("Usage: adapter_record_command.py -- <command>", file=sys.stderr)
        return 1
    ...
    try:
        r = subprocess.run(
            command_args, capture_output=True, text=True, timeout=3600,
        )
